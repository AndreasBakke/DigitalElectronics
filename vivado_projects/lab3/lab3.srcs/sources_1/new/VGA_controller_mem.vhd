...
ARCHITECTURE Behav of VGA_controller_mem IS
    ...
    CONSTANT a_h_count : integer := 95; ---targetcount (0 indexed)
    CONSTANT b_h_count : integer := 47;
    CONSTANT c_h_count : integer := 639;
    CONSTANT d_h_count : integer := 16;
    
    CONSTANT a_v_count : integer := 1; --targetcount (0 indexed counting)
    CONSTANT b_v_count : integer := 32;
    CONSTANT c_v_count : integer := 479;
    CONSTANT d_v_count : integer := 9;
...
BEGIN
    ...
    rst_n <= KEY(0) AND l; -- Reset if either locked from pll or key_0 is 0
	 ...
	 mem_rom_r_1: img_rom_r PORT MAP(aclr => rst_n, address => mem_addr, clock => clk, rden => mem_enable, q=>mem_r);
	 mem_rom_g_1: img_rom_g PORT MAP(aclr => rst_n, address => mem_addr, clock => clk, rden => mem_enable, q=>mem_g);
	 mem_rom_b_1: img_rom_b PORT MAP(aclr => rst_n, address => mem_addr, clock => clk, rden => mem_enable, q=>mem_b);
